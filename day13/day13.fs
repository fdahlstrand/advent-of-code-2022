...
type PacketElement = | List of Packet | Number of int
...
input |> Seq.map compare |> Seq.mapi (fun i v -> i+1, v) |> Seq.filter (fun p -> snd p = -1 ) |> Seq.sumBy( fun p -> fst p) |> printfn "%d" 